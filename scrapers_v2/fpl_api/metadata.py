...
def scrape_season_data():
    ...
    # ====== Players Season data ======
    players_data_df[players_data_df["id"] == 2].to_dict(orient="records")  # type: ignore
    ...
    players_season_data.to_csv(
        get_data_path(SEASON, "players_season_api.csv"), index=False
    )

    # ====== Team join date df ======
    team_join_dates = pd.DataFrame(data["elements"])
    team_join_dates = team_join_dates[["id", "team_join_date"]]
    team_join_dates.rename(columns={"id": "player_id"}, inplace=True)  # type: ignore
