...
def main():
    ...
    merged_gw = pd.merge(
        merged_gw,
        team_stats,
        left_on=["opponent_team_id", "was_home"],
        right_on=["team_id", "was_home"],
        how="left",
        suffixes=("", "_opponent"),
    )
    ...
    season_data.drop(columns=["team_id", "web_name"], inplace=True)
    season_data.columns = [
        col + "_season" if col not in ["player_id", "element_type"] else col
